...
namespace DAL
{
    public static class Session
    {
        ...
        public static person GetValidUser
        {
            get
            {
                if (User == null)
                {
                  var vUser  = new person();
                  vUser.benutzername = "Kein user angemeldet";
                  vUser.email = Session.EMailZahlungErfolgt;
                  vUser.id = 0;
                  return vUser;
        ...
        public static string DatabaseVersion
        {
            get
            {
                try
                {
                    ...
                    {
                         return db.config.Where(s => s.mkey == "DatabaseVersion").FirstOrDefault().value;
                    }
            ...
            get
            {
                try
                {
                    using (var db = new SteinbachEntities())
                    {
                        ...
                        _LogState = int.Parse(res.value.FirstOrDefault().ToString());
        ...
        public static bool ProjekteAufklappen
        {
            get
            {
                ...
                {
                    DAL.Tools.LoggingTool.LogExeption(ex, "Sesseion.cs", "ProjekteAufklappen", LoggingTool.LogState.high);
                    return true;
                }
            ...
            get
            {
                try
                {
                    using (var db = new SteinbachEntities())
                    {
                        ...
                        return int.Parse(res.value.FirstOrDefault().ToString()) == 0 ? false : true;
                    }
                }
                catch (Exception ex)
                {
                    DAL.Tools.LoggingTool.LogExeption(ex, "Sesseion.cs", "Testmodus", LoggingTool.LogState.high);
                    return false;
                }
        ...
        public static long TerminTimerIntervalSMTP
        {
            get
            {
                try
                {
                    using (var db = new SteinbachEntities())
                    {
                        var res = db.config.Where(s => s.mkey == "TerminTimerInterval").FirstOrDefault();
                        return long.Parse(res.value);
                    }
                }
                catch (Exception ex)
                {
                    DAL.Tools.LoggingTool.LogExeption(ex, "Session.cs", "TerminTimerIntervalSMTP", LoggingTool.LogState.high);
                    return 900;
                }
